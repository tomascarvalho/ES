...
def user_tickets_details(rt_object, email):
    ...
    if email == 'dir' or email == 'main' or email == "statistic" or email == "new_ticket" :
    ...
    try:
        ...
        for i in response:
            i.update({'description': getTicketDescription(rt_object, i['id'])}) ## ADICIONA A DESCRIPTION ##
    except ValueError as e:
        response = []

    if email == 'dir' or email == 'dir-inbox' or email == 'unknown' or email == 'history':
...
def search_tickets(rt_object, search):
    ...
    try:
        response = get_list_of_tickets(rt_object, query)
        for i in response:
            i.update({'description': getTicketDescription(rt_object, i['id'])})
            print ("Debug")
    except NameError as e:
        raise ValueError('Error: '+str(e))
    except ValueError:
...
def create_ticket(rt_object, subject, text):
    new_ticket = {
        'id' : 'ticket/new',
        'Queue' : 'General',
        'Owner' : 'nobody',
    ...
    create(rt_object,new_ticket)

def create_child_ticket(rt_object, subject, text):
    new_ticket = {
        'id' : 'ticket/new',
        'Queue' : 'General',
        'Owner' : 'nobody',
